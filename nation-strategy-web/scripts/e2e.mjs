...
// ---------------------------------------------------------------- desktop interactions
{
  ...
  ok('story step 2 lights Identity box', (await page.textContent('.nid-box.is-lit')).includes('Identity'))
  await page.check('.anon-toggle input')
  ok('anonymous toggle changes story', (await page.textContent('.story-line')).includes('ไม่ Login') && !!(await page.$('.nid-core.is-anon')))
  ...
  await page.keyboard.press('Escape')
  await page.click('.nid-box:has-text("Interest")')
  ok('Interest box shows cautions', (await page.textContent('.panel')).includes('ไม่อนุมาน'))
  await page.keyboard.press('Escape')
